...
class ViewController: UIViewController {
    ...
    @objc func routeButtonTapped() {
       ...
       for index in stride(from: 0, through: annotationsArray.count - 2, by: 1) {
            
            crateDirectionRequest(startCoordinate: annotationsArray[index].coordinate, destinationCoordination: annotationsArray[index + 1].coordinate)
        }
    ...
    private func setupPlacemark(adressPlace: String) {
        ...
        geocoder.geocodeAddressString(adressPlace) { [self] (placemarks, error) in
            if let error = error{
                print(error)
                alertError(title: "Ошибка", message: "Сервер не доступен. Попробуйте добавить адрес ещё раз")
                return
            }
    ...
    private func crateDirectionRequest(startCoordinate: CLLocationCoordinate2D, destinationCoordination: CLLocationCoordinate2D) {
